...
| **AnimationTree** | State machine with blends/transitions (character with many states) | AnimationTree |
| **Tween** | One-off procedural animations (damage flash, bounce, fade) | Code-only |

## Step 2 — AnimatedSprite2D Setup (Frame Animations)

For characters/entities with sprite sheet animations:
...
**Note:** SpriteFrames resources must be configured in the Godot editor — Claude cannot assign individual sprite frames from sprite sheets programmatically. Report this to the user.
