...
func _setup_npc_dialogue() -> void:
	...
	if _garrick_npc and _garrick_npc.visible:
		_garrick_npc.dialogue_lines = get_garrick_casual_dialogue(flags)
...
static func get_maren_dialogue(
	...
	if flags.get("garrick_recruited", false):
		return PackedStringArray([
			"Old Iron himself, traveling with my favorite Echo hunter."
			+ " Now that's a sight.",
			"Garrick used to pass through years ago. Always ordered"
			+ " the same thing \u2014 black tea, no sugar."
	...
	if flags.get("iris_recruited", false):
		return PackedStringArray([
			...
			"Don't worry, I don't pry. Anyone who fights alongside"
			+ " Kael is welcome at my table.",
	...
	if flags.get("opening_lyra_discovered", false):
		return PackedStringArray([
			...
			"You found something in the ruins? Something... alive?",
			"I won't pretend to understand echoes the way you do."
			+ " But if this one can speak, that changes things.",
			"The world's been holding its breath for three hundred"
			+ " years, Kael. Maybe it's finally ready to exhale.",
			"Rest here tonight. Whatever comes next,"
			+ " you'll face it better on a full stomach.",
		])
	return PackedStringArray([
		"Kael! Come in, come in. You look like you haven't eaten"
		+ " since yesterday. ...You haven't, have you?",
		"The stew's still warm. Sit down before you fall down.",
		...
		+ " creatures. Stay close to the forest paths, will you?",
	])
...
static func get_bram_dialogue(
	...
	if flags.get("garrick_recruited", false):
		return PackedStringArray([
			"Garrick Thorne is with you? THE Garrick Thorne?"
			+ " The man's a legend around here.",
			"He saved a caravan from crystal-corrupted wolves ten"
			+ " years back. Single-handedly held the pass"
			+ " while they escaped.",
	...
	if flags.get("opening_lyra_discovered", false):
		return PackedStringArray([
			"Everyone's talking about what you found in the ruins."
			+ " A conscious echo?"
			+ " That's... that's not supposed to happen, is it?",
			"I don't like it, Kael. Change is coming, and change"
			+ " is bad for business. Change is bad for everything.",
			"The caravan still hasn't arrived."
			+ " I'm starting to think something happened"
			+ " on the road.",
			"If you run into any traders out there, tell them"
			+ " Roothollow is paying double for medical supplies."
			+ " Triple, even. I don't care.",
		])
	return PackedStringArray([
		"Oh, Kael. Good timing. Well... actually,"
		+ " terrible timing.",
		"The supply caravan from Prismfall is three days late."
		+ " Three days! That's never happened before.",
		"I've got some basic provisions left, but the good"
		+ " equipment? Gone. Bought up by a group of hunters"
		+ " heading south.",
		"If you're heading to the ruins, I can spare a couple"
		+ " of salves. It's not much, but it's what I've got.",
	])
...
static func get_thessa_dialogue(
	...
	if flags.get("iris_recruited", false):
		return PackedStringArray([
			...
			"The fact that she left the Initiative tells me more"
			+ " about her character than anything she could say."
			+ " It takes courage to walk away from power.",
			"But be careful. The Initiative doesn't let its assets"
			+ " go quietly. If they're looking for her,"
			+ " they may eventually look here.",
		])
	if flags.get("opening_lyra_discovered", false):
		return PackedStringArray([
			"A conscious echo. I've read theories... fragments of"
			+ " old research papers recovered from the capital."
			+ " But I never believed it possible.",
			"Do you know what this means, Kael? Echoes are"
			+ " crystallized memory \u2014 fragments of lives lived"
			+ " and lost. If one has achieved consciousness...",
			"...then the boundary between what was and what is"
			+ " may be thinner than we thought.",
			"The Shepherds of Silence would destroy her on sight."
			+ " The Reclamation Initiative would cage her and"
			+ " study her. Neither can learn of this.",
			"Protect her, Kael. And listen to what she has to say."
			+ " The dead don't speak without reason.",
		])
	return PackedStringArray([
		"Ah, Kael. I was wondering when you'd visit."
		+ " The crystals in my study have been humming"
		+ " all morning.",
		"You're heading to the ruins again. I can see it in"
		+ " your eyes \u2014 that restless look you get"
		+ " when the echoes call.",
		"Before you go, a word of caution. The echoes in the"
		+ " old capital have been... different lately."
		+ " More coherent. Almost purposeful.",
		"In all my years studying Resonance, I've never felt"
		+ " anything like it. It's as if something buried"
		+ " is trying to wake up.",
		"Trust your instincts out there. You've always had an"
		+ " unusual connection to the echoes."
...
static func get_wren_dialogue(
	...
	if flags.get("garrick_recruited", false):
		return PackedStringArray([
			...
			"With him, Iris, and you? You might actually survive"
			+ " what's out there. High praise from me.",
		])
	if flags.get("iris_recruited", false):
		return PackedStringArray([
			"Your new companion handles herself well. I watched"
			+ " her take down a crystal-shard serpent near the"
			+ " forest edge without breaking stride.",
			"That arm of hers packs a punch. Literally."
			+ " The serpent didn't know what hit it.",
			"Good. You're going to need someone who can fight."
			+ " The creatures between here and the ruins"
			+ " aren't getting any friendlier.",
		])
	if flags.get("opening_lyra_discovered", false):
		return PackedStringArray([
			"You came back from the ruins looking like you'd seen"
			+ " a ghost. Or... whatever's worse than a ghost.",
			"Look, I don't need details. But if something's"
			+ " changing in there, I need to know."
			+ " My job is keeping this village safe.",
			"The Verdant Forest has been restless since you got"
			+ " back. More echo activity, more corrupted beasts."
			+ " Like something stirred them up.",
			"If you're going out again, stick to the main paths."
			+ " And maybe bring a friend or two.",
		])
	return PackedStringArray([
		"Heading out? The western trail's clear, but I wouldn't"
		+ " stray too far south. Saw tracks.",
		"Big ones. Not wolves. Something... wrong."
		+ " Crystal growths where the paw prints should be.",
		"The forest is getting worse. Used to be you'd see a"
		+ " corrupted creature once a month."
...
static func get_garrick_casual_dialogue(
	...
	if flags.get("opening_lyra_discovered", false):
		return PackedStringArray([
			...
			"A conscious echo... I once served people who would"
			+ " have called that an abomination."
			...
			+ " Not the kind who smile when they want something.",
		])
	return PackedStringArray([
		...
		"This village... it's peaceful. Reminds me of places"
		+ " that don't exist anymore. Enjoy it while it lasts.",
	])
...
static func get_lina_dialogue(
	...
	if flags.get("garrick_recruited", false):
		return PackedStringArray([
			"The big man with the shield told me a story about a"
			+ " knight who fought a dragon made of memories!",
	...
	if flags.get("opening_lyra_discovered", false):
		return PackedStringArray([
			...
			"Mama told me to throw it away, but I hid it under"
			+ " my pillow instead. Don't tell, okay?",
		])
	return PackedStringArray([
		"Kael! Kael! Look what I found by the river!",
		"It's a pretty rock. See how it shines? Mama says it's"
		+ " just quartz, but I think it's an echo. A tiny one.",
		"When I hold it up to my ear, I can almost hear someone"
		+ " singing. Is that weird?",
		"When I grow up, I want to be an Echo hunter like you!"
		+ " I'll have a big journal and everything!",
	])
