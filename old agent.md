...
Agent names: Use `Codex` or `gemini` as the assignee value.
...
- The AGENTS.md project instructions reload automatically after clear, so all rules and conventions are preserved
...
5. **Write tests FIRST** (TDD) — see `game/tests/AGENTS.md`
...
| Assets | `game/assets/AGENTS.md` + `04-resources-and-data.md` |
| Tilemaps | `11-tilemaps-and-level-design.md` + `tilemap-builder` agent |

## MANDATORY: Test-Driven Development (TDD)

**Tests come FIRST, implementation comes SECOND.** Full details in `game/tests/AGENTS.md`.
...
```
game/              # Godot project — each subdirectory has its own AGENTS.md
  ...
  assets/          # Art + audio (gitignored PNGs/audio, see game/assets/AGENTS.md)
...
assets/            # Asset index and sprite catalog (AGENTS.md)
```

**Each `game/` subdirectory has its own `AGENTS.md`** with file indexes, APIs, conventions, and usage patterns. Read the relevant subdirectory AGENTS.md before modifying files in that directory.

## Art Assets

**ALWAYS search the Time Fantasy packs before generating placeholder art.** Packs are at `/Users/robles/repos/games/assets/`. Full pack index: `assets/AGENTS.md`.

For asset copy workflow, gitignore rules, `load()` null-check patterns, and tile sheet reference: see `game/assets/AGENTS.md`.

When importing tilesets, **copy ALL tile sheets from the pack** (not just the one you need). Use `/copy-assets` to automate.

## Tilemap Design

Read `docs/best-practices/11-tilemaps-and-level-design.md` before any tilemap work. Scene-specific tilemap details are in `game/scenes/AGENTS.md`. Detailed visual-first workflow is in `.Codex/agents/tilemap-builder.md`.
...
**Every change to visual content MUST be verified with `/scene-preview` before committing.** Details in `game/tools/AGENTS.md`.
...
See `game/data/AGENTS.md` for the .tres template and `game/resources/AGENTS.md` for the EnemyData schema. Workflow:
