...
Right now you're seeing Godot's default window size and texture filtering, so the icon might look a little soft. Don't worry about that yet. The 1280x720 window and crisp pixels show up after the next section, once we apply the Pixel Art settings.
...
If these defaults clash with something else on your system, you can remap them under **Editor → Editor Settings → Shortcuts**.
...
### Display Settings
First, flip on the **Advanced Settings** toggle in the top-right corner. Without it, Godot hides the override and texture-filter options you're about to need. The width and height overrides live under **Display → Window → Size**.
